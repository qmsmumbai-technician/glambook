...
5. **Panel icon** (top right, grid icon) → seven tools:
   ...
   - **Inventory** — two tabs, **Consumable Items** and **Non-consumable Items**. Each item has a description plus **Stock**, **Min Qty**, and **Max Qty**. Tap **+ Add item** to add more — the list grows as you enter items. Whenever Stock drops below Min Qty for anything, that number turns red right there in the list, and a banner appears at the top ("N items below Min Qty — View Reorder Sheet"). Tapping it opens the **Reorder Sheet** — every low-stock item listed with **Item Desc.** and **Order Qty** (calculated as Min Qty − Stock), with **Send via WhatsApp/SMS** to send the list straight to a supplier (opens a contact picker, since there's no saved supplier number).
